  ...
  stages {
    ...
    stage('Child Jobs') {
      steps {
        ...
        build quietPeriod: 6, wait: true, job: 'f1'
        build quietPeriod: 6, wait: false, job: 'm1'
        build quietPeriod: 6, wait: false, job: 'm2'
        build quietPeriod: 6, wait: false, job: 'f0'
      }
    }
    stage('Hello_12') {
      steps {
        echo 'Hello World'
        checkout scmGit(branches: [[name: '*/main']], extensions: [], userRemoteConfigs: [[credentialsId: 'Gitlab', url: 'https://gitlab.com/bharath_devops1/p_1.git']])
      }
    }
  }
  post {
